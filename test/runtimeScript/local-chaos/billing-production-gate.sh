...
billing_production_gate_resolve_relay() {
  ...
  node_id=$(node "$ROOT_DIR/test/runtimeScript/local-chaos/node-id-from-key.mjs" \
...
billing_production_gate_read_accounting() {
  ...
  CA_BASE_URL="http://127.0.0.1:$ca_port" \
    ...
    node "$ROOT_DIR/test/runtimeScript/local-chaos/ca-web-accounting.mjs" \
      "$relay_id" "$payer_id" "$relay" "$payer" 2>/dev/null
}

billing_production_gate_read_nat_observation() {
  local server=$1
  node "$ROOT_DIR/test/runtimeScript/local-chaos/nat-billing-private-inspect.mjs" \
