...
SOAK_HOME=${BNFS_SOAK_HOME:-$ROOT_DIR/test/runtimeScript/local-chaos/.soak}
...
source "$STABILITY_SUPPORT_ROOT/test/runtimeScript/local-chaos/billing-adversary-gate.sh"
source "$STABILITY_SUPPORT_ROOT/test/runtimeScript/local-chaos/billing-production-gate.sh"
source "$STABILITY_SUPPORT_ROOT/test/runtimeScript/local-chaos/mixed-path-gate.sh"
source "$STABILITY_SUPPORT_ROOT/test/runtimeScript/local-chaos/network-pools.sh"
...
worker_runtime_snapshot_files() {
  printf '%s\n' \
    test/runtimeScript/local-chaos-stability.sh \
    test/runtimeScript/local-chaos/billing-adversary-gate.sh \
    test/runtimeScript/local-chaos/billing-production-gate.sh \
    test/runtimeScript/local-chaos/mixed-path-gate.sh \
    test/runtimeScript/local-chaos/network-pools.sh \
    test/runtimeScript/local-chaos/lib.sh
}
...
prepare_worker_runtime_snapshot() {
  ...
  chmod 500 "$staging_root/test" "$staging_root/test/runtimeScript" \
    "$staging_root/test/runtimeScript/local-chaos" || {
    rm -rf "$staging_root"
    return 1
  }
  ...
  printf '%s\n' "$snapshot_root/test/runtimeScript/local-chaos-stability.sh"
}
...
registered_random_worker() {
  ...
  if [[ $client == batch-scheduler ]]; then
    exec bash "${STABILITY_WORKER_SCRIPT:-$ROOT_DIR/test/runtimeScript/local-chaos-stability.sh}" \
...
provision_container_adversaries() {
  ...
  PRIVATE_RUNTIME_DIR="$PRIVATE_RUNTIME_DIR" \
    ...
    node "$ROOT_DIR/test/runtimeScript/local-chaos/provision-adversaries.mjs" \
...
node_id_from_private_key() {
  node "$ROOT_DIR/test/runtimeScript/local-chaos/node-id-from-key.mjs" "$1"
}
  ...
  while (( $(date +%s) < deadline_epoch )); do
    ...
    while IFS=$'\t' read -r client relay relay_endpoint family listen_port node_id \
      ...
      BNFS_RANDOM_BATCH_ID="$batch_id" BNFS_RANDOM_BATCH_MEMBER=1 \
        ...
        bash "${STABILITY_WORKER_SCRIPT:-$ROOT_DIR/test/runtimeScript/local-chaos-stability.sh}" \
          _worker "$run_dir" "$client" \
...
run_internal() {
  ...
  if ! BNFS_CHAOS_RUNTIME_DIR="$run_dir/build-runtime" BNFS_CHAOS_COMPOSE_PROJECT="$project-build" \
    ...
    bash "$ROOT_DIR/test/runtimeScript/local-deploy-test.sh" --build-only > "$run_dir/build.log" 2>&1; then
  ...
  BNFS_CHAOS_ENABLE_CA=1 BNFS_CHAOS_ENABLE_ADVERSARIES="$enable_container_adversaries" BNFS_CHAOS_CA_HOST_PORT="$ca_port" \
    ...
    node "$ROOT_DIR/test/runtimeScript/local-chaos/generate-compose.mjs" "$RUNTIME_DIR" > "$COMPOSE_FILE"
  ...
  setsid env BNFS_RECONNECT_GATE_TOKEN="$reconnect_gate_token" \
    node "$ROOT_DIR/test/runtimeScript/local-chaos/reconnect-gate.mjs" \
  ...
  if ! topology_reset || ! wait_service_health ca 45; then
    ...
    setsid "$ROOT_DIR/test/runtimeScript/local-chaos/resource-guard.sh" \
...
if [[ ${BASH_SOURCE[0]} == "$0" ]]; then
  case ${1:-status} in
    ...
    _worker)
      shift
      source "$STABILITY_SUPPORT_ROOT/test/runtimeScript/local-chaos/lib.sh"
