...
mkdir -p "$source_root/test/runtimeScript/local-chaos" "$run_dir"

while IFS= read -r relative_file; do
  destination="$source_root/$relative_file"
  mkdir -p "$(dirname "$destination")"
  if [[ $relative_file == test/runtimeScript/local-chaos-stability.sh ]]; then
...
[[ $snapshot_script == "$snapshot_root/test/runtimeScript/local-chaos-stability.sh" ]] \
...
printf '%s\n' '#!/usr/bin/env bash' 'printf "' > "$source_root/test/runtimeScript/local-chaos-stability.sh"
printf '%s\n' '#!/usr/bin/env bash' 'if [[ "' > "$source_root/test/runtimeScript/local-chaos/lib.sh"
if bash -n "$source_root/test/runtimeScript/local-chaos-stability.sh" >/dev/null 2>&1; then
  fail 'mutated source worker unexpectedly remained syntactically valid'
fi
if bash -n "$source_root/test/runtimeScript/local-chaos/lib.sh" >/dev/null 2>&1; then
