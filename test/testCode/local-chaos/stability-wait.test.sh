...
mkdir -p "$registry_dir" "$fake_root/test/runtimeScript"
printf '#!/usr/bin/env bash\nexit 0\n' > "$fake_root/test/runtimeScript/local-chaos-stability.sh"
...
REAL_ROOT=$ROOT_DIR FAKE_ROOT=$fake_root RUN_DIR_FOR_TEST=$registry_dir \
  ...
  setsid env BNFS_RANDOM_WORKER_TOKEN="$registry_token" bash -c '
    source "$REAL_ROOT/test/runtimeScript/local-chaos-stability.sh"
    ROOT_DIR=$FAKE_ROOT
