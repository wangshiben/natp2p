...
run_case() {
  ...
  setsid bash "$ROOT_DIR/test/runtimeScript/local-chaos/resource-guard.sh" \
...
run_deadline_cleanup_failure_case() {
  ...
  TEST_DOCKER_LEAK=1 setsid bash "$ROOT_DIR/test/runtimeScript/local-chaos/resource-guard.sh" \
