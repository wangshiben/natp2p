...
## 1.5 真实跨境链路验证结果（已执行 2026-06-28）

拓扑：client(server3 大陆 129.0.0.1) → relay1(server1 香港 38.0.0.1:9000)
→ 跨中继桥接池 → relay2(server2 美国 104.0.0.1:9001) → tunserver → http(20MB)。

| 验证点 | 结果 |
|--------|------|
| 端到端完整性 | ✅ 20MB(20971520B)经隧道下载，SHA256 与源端一致(`686fc5d9...`)，速率 ~96KB/s（跨境拥塞链路） |
| KCP 阻断降级 | ✅ 大陆→香港 KCP(UDP) 不通，自动双 TCP failover（日志 `KCP 不通, 改用第二条 TCP leg`），端到端不受影响 |
| **连接复用** | ✅ 10 并发 HTTP 请求 → relay1→relay2 仅 **1 条物理 TCP**（改造前为每会话一条）；日志 `新建连接池 minWarm=1` |
| **扩容** | ✅ 多会话并发达高水位 → 三次扩容 `新增 1 条物理连接 (now=2/8 → 3/8 → 4/8, burst=false)` |
| **收缩** | ✅ 负载回落后 reaper 逐周期回收 `回收 1 条空闲物理连接 (剩余=4 → 剩余=3)`，保底 minWarmConns=1 |

**验证中发现并修复的真实缺陷**（提交 d884365）：
client 异常断开（kill -9 / RST）后，`pumpLocalToPeer` 读到 EOF 仅退出 goroutine 而未关闭
对端 mux stream → 对端 relay 的 `activeStreams` 永不归零 → 该 physConn 被孤儿 stream 钉住、
reaper 无法回收（实测池停在 3 条无法收敛到保底 1）。修复：`CrossRelayBridge` 增 `liveLegs`
计数，最后一条 local leg 结束时关闭 `peerConn`（池化下只关该逻辑会话，不影响共享物理连接；
liveLegs 确保 dual-leg failover 时不误切存活 leg）。修复后单 client 干净生命周期池稳定保底=1。

**未能在此链路复现的项**：≥51 并发独立桥接会话的高峰批量扩容（`burst=true`）——
拥塞的跨境链路无法同时承载几十条新建会话的握手（`context deadline exceeded`），
此为链路带宽/握手超时限制，非池逻辑问题；高峰批量扩容已由本地单测 `BurstDetector`/
`BurstBatchTrim` 覆盖。
...
- 真实跨境链路验证（§1.5）已执行：复用/扩容/收缩/KCP 降级/端到端完整性全部确认，
  并发现+修复一处真实泄漏（孤儿 stream 钉住 physConn，提交 d884365）。
  剩余仅 ≥51 并发高峰批量扩容因链路带宽受限未在真链复现（已由本地单测覆盖）。
