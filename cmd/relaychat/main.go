...
三机端到端示例(server1=index, server2=relay, server3=server/callee, 本机=client):
  # server1 (38.76.170.102) 起 index
  relaychat -mode index -listen 0.0.0.0:9000 -public 38.76.170.102:9000
  # server2 (104.252.30.204) 起 relay 并注册到 index
  relaychat -mode relay -listen 0.0.0.0:9000 -public 104.252.30.204:9000 -index 38.76.170.102:9000
  # server3 (129.28.42.247) 起 callee, 经 index bootstrap(试 server2 不可达→回退 index), 记下 NodeID
  relaychat -mode listen -index 38.76.170.102:9000
  # 本机 client 经 index bootstrap(命中 server2), 跨中继连接 callee, 3 轮通信
  relaychat -mode connect -index 38.76.170.102:9000 -target <NodeID> -rounds 3 -msg hello
